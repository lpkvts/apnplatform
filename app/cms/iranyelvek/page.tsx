...
interface Row { id: string; title: string; status: string; ai_generated: boolean; review_on: string | null; expires_on: string | null }
...
export default async function IranyelvekPage() {
  const { role } = await currentRole()
  if (!isStaff(role)) return <><h1 className="h1">Irányelvek</h1><div className="card">Ehhez szerkesztő/lektor/admin jogosultság szükséges.</div></>
  const canPublish = PUBLISHERS.includes(role as Role)
  const today = new Date().toISOString().slice(0, 10)
  const supabase = await createClient()
  const { data } = await supabase.from('guidelines').select('id, title, status, ai_generated, review_on, expires_on').order('status').order('title').returns<Row[]>()
  const items = data ?? []
  return (
    <>
      <Link className="sh-back" href="/cms">‹ Tartalomkezelés</Link>
      <div className="row" style={{ border: 'none' }}>
        <h1 className="h1" style={{ margin: 0 }}>Irányelvek</h1>
        <Link className="btn sm" href="/cms/uj">+ Új</Link>
      </div>
      ...
      {items.length === 0 && <p className="sub">Még nincs irányelv.</p>}
    </>
  )
}
