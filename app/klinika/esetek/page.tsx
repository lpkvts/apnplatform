...
export default async function CasesPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const chip = (val: string, label: string) => (
    <Link href={val ? `/klinika/esetek?status=${val}` : '/klinika/esetek'} className={`sh-chip ${(status ?? '') === val ? 'on' : ''}`}>{label}</Link>
  )
  ...
  return (
    <>
      ...
      <div className="row" style={{ border: 'none' }}>
        <h1 className="h1" style={{ margin: 0 }}>Saját klinikai eseteim</h1>
        <form action={createCase}><button className="btn sm" type="submit">+ Új eset</button></form>
      </div>

      <div className="sh-chips">
        {chip('', 'Összes')}{FILTERS.map((f) => chip(f, STATUS_LABEL[f]))}
      </div>

      {items.length === 0 ? (
        <div className="card"><p style={{ margin: 0 }}>Nincs eset ebben a nézetben.</p>
          <p className="sub" style={{ marginBottom: 0 }}>Indíts egy új klinikai esetet a jobb felső gombbal.</p></div>
      ) : (
        items.map((c) => (
          <Link key={c.id} className="sh-row" href={`/klinika/esetek/${c.id}`}>
            <span className="sh-row-main">
              <span className="sh-row-name">CASE #{String(c.case_no).padStart(6, '0')} · {c.title}</span>
              <span className="sh-row-sub">{STATUS_LABEL[c.status]} · {new Date(c.updated_at).toLocaleDateString('hu-HU')}{c.complaint ? ` · ${c.complaint}` : ''}</span>
            </span>
