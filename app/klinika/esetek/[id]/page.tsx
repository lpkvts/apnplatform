...
interface CaseRow {
  ...
  summary: string | null; sbar: Sbar | null; problems: string[] | null; red_flags: string[] | null; decision: string | null
}

function RemoveBtn({ table, id, caseId }: { table: string; id: string; caseId: string }) {
  return (
    <form action={removeCaseChild} style={{ display: 'inline' }}>
      <input type="hidden" name="table" value={table} /><input type="hidden" name="id" value={id} /><input type="hidden" name="case_id" value={caseId} />
      <button className="sh-back" type="submit" style={{ padding: 0 }}>✕</button>
    </form>
  )
}
...
export default async function CaseDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <form action={addCaseScore} className="as-vitrow" style={{ marginTop: 6 }}>
        <input type="hidden" name="case_id" value={c.id} />
        <select className="as-vin" name="score_id" defaultValue="" style={{ minWidth: 130 }} required>
          <option value="" disabled>Score…</option>
          {TESTS.map((t) => <option key={t.id} value={t.id}>{t.abbr || t.name}</option>)}
        </select>
        <input className="as-vin" name="value" type="text" inputMode="decimal" placeholder="pont" style={{ maxWidth: 80 }} />
        <input className="as-vin" name="band" type="text" placeholder="sáv (opc.)" style={{ maxWidth: 100 }} />
        <button className="btn sm" type="submit">Hozzáad</button>
      </form>
      ...
      <form action={addCaseEkg}>
        <input type="hidden" name="case_id" value={c.id} />
        <div className="as-vitrow" style={{ marginTop: 6 }}>
          <select className="as-vin" name="ekg_id" defaultValue="" style={{ minWidth: 150 }} required>
            <option value="" disabled>EKG lelet…</option>
            {ECG.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}
          </select>
          <input className="as-vin" name="note" type="text" placeholder="megjegyzés (opc.)" />
          <button className="btn sm" type="submit">Hozzáad</button>
        </div>
      </form>
