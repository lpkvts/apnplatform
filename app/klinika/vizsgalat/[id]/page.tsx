...
interface Session { id: string; title: string; mode: string; exam_type: string | null; focus: string | null; status: string; anamnesis: Record<string, unknown>; vitals: Record<string, string>[]; general_exam: Record<string, unknown>; systems: Record<string, Record<string, string | string[]>>; red_flags: string[]; summary: string | null }
...
export default async function VizsgalatDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ sec?: string }> }) {
  ...
  const { data } = await supabase.from('exam_sessions').select('id, title, mode, exam_type, focus, status, anamnesis, vitals, general_exam, systems, red_flags, summary').eq('id', id).maybeSingle<Session>()
  if (!data) notFound()
  ...
  return (
    <>
      ...
      {sec === 'osszegzes' && (
        <ExamSummaryForm id={id} data={data} />
      )}
