...
export default function KlinikaiTudastarHub() {
  return (<><Link className="sh-back" href="/klinika">‹ Klinikai mag</Link><h1 className="h1">Klinikai Tudástár</h1><p className="sub">Betegségek, klinikai állapotok, tünetek és evidence-alapú szakmai tartalmak egy helyen.</p><ClinicalDisclaimer />{SECTIONS.map((s) => (<Link key={s.title} className="card klink" href={s.href}><div className="klink-t">{s.icon} {s.title}</div><div className="sub" style={{ margin: '4px 0 0' }}>{s.sub}</div></Link>))}</>)
}
