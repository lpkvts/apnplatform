...
export const PANELS: { name: string; ids: string[] }[] = [
  { name: 'Vérkép', ids: ['hb', 'wbc', 'plt', 'rbc', 'htk', 'mcv', 'mch', 'rdw'] },
  { name: 'Vörösvérkép', ids: ['hb', 'rbc', 'htk', 'mcv', 'mch', 'mchc', 'rdw'] },
  { name: 'Vaspanel', ids: ['hb', 'mcv', 'rdw', 'ferr', 'tsat', 'vas', 'tvk'] },
  { name: 'Hemolízis', ids: ['hb', 'retik', 'retabs', 'ldh', 'hapto', 'bili'] },
...
export const PATTERNS: Pattern[] = [
  {
    ...
    further: ['Ferritin, transzferrin-szaturáció', 'Szérumvas, TVK', 'RDW (magas RDW erősíti a vashiányt)', 'Retikulocita', 'Széklet vér (okkult)'],
  ...
  {
    id: 'hemolysis', name: 'Hemolízis mintázata', sev: 'sürgős',
    test: (d) => (hi(d, 'retik') || hi(d, 'retabs')) && hi(d, 'ldh') && (lo(d, 'hapto') || hi(d, 'bili')),
    summary: 'Emelkedett retikulocita és LDH, csökkent haptoglobinnal vagy emelkedett bilirubinnal — fokozott vörösvértest-pusztulással (hemolízissel) összeegyeztethető mintázat.',
    apnFocus: ['Sárgaság, sclera- és bőrszín figyelése', 'Vizeletszín (sötét, kólaszínű) dokumentálása', 'Vitális paraméterek, transzfúziós előzmény tisztázása'],
    further: ['Haptoglobin, direkt/indirekt bilirubin', 'Vérkenet (schistocyta, sphaerocyta!)', 'Direkt Coombs-teszt', 'Thrombocytaszám, kreatinin'],
    consult: 'Azonnal, ha friss transzfúzió zajlott, illetve thrombocytopenia vagy vesefunkció-romlás társul (TTP/HUS gyanú).',
    guidelineKw: ['anaemia', 'hemolizis', 'hemolízis'],
  },
  {
    id: 'tma', name: 'Thromboticus mikroangiopátia gyanúja', sev: 'sürgős',
    test: (d) => lo(d, 'plt') && hi(d, 'ldh') && (lo(d, 'hb') || lo(d, 'hapto')),
    summary: 'Alacsony thrombocytaszám magas LDH-val és anaemiával/alacsony haptoglobinnal — TTP, HUS vagy DIC irányába terelő mintázat; a vérkenetben schistocyta megerősítheti.',
    apnFocus: ['Neurológiai állapot és tudat figyelése', 'Diurézis, vérnyomás követése', 'Vérzésjelek dokumentálása', 'Invazív beavatkozás előtt egyeztetés'],
    further: ['Vérkenet schistocytára', 'Kreatinin, D-dimer, fibrinogén', 'Direkt Coombs-teszt'],
    consult: 'Azonnal — sürgős orvosi értékelés, a kórkép kezelése időkritikus.',
  },
  {
    id: 'hypoprolif', name: 'Hipoproliferatív anaemia mintázata', sev: 'figyelendő',
    test: (d) => lo(d, 'hb') && (lo(d, 'retik') || lo(d, 'retabs')),
    summary: 'Alacsony Hb elégtelen retikulocita-válasszal — a csontvelő nem termel megfelelően: hiányállapot, krónikus betegség, vesebetegség vagy csontvelő-szuppresszió lehetősége.',
    apnFocus: ['Táplálkozási anamnézis', 'Csontvelő-toxikus gyógyszerek áttekintése', 'Terhelhetőség, eséskockázat felmérése'],
    further: ['Ferritin, transzferrin-szaturáció', 'B12, folsav', 'Vesefunkció (eGFR), TSH'],
    consult: 'Súlyos anaemia, pancytopenia vagy tisztázatlan ok esetén.',
    guidelineKw: ['anaemia'],
  },
  {
    id: 'mixed_def', name: 'Kevert hiányállapot gyanúja', sev: 'info',
    test: (d) => hi(d, 'rdw') && d.mcv === 'normal',
    summary: 'Magas RDW normális MCV mellett — a sejtpopuláció egyenetlen, ami korai vagy kevert (vas és B12/folsav együttes) hiányra utalhat, mert a mikro- és makrocitás sejtek átlaga normálisnak látszik.',
    apnFocus: ['Táplálkozási anamnézis', 'Alkoholfogyasztás tisztázása', 'Felszívódási zavarra utaló panaszok'],
    further: ['Ferritin, transzferrin-szaturáció', 'B12, folsav', 'Vérkenet (aniso-, poikilocytosis)'],
    consult: 'Ha anaemia is társul, vagy a hiány pótlás mellett sem rendeződik.',
  },
  {
    id: 'thal_susp', name: 'Talasszémia-vonás lehetősége', sev: 'info',
    test: (d) => lo(d, 'mcv') && d.rdw === 'normal' && (d.ferr === 'normal' || d.ferr === 'high' || d.rbc === 'high'),
    summary: 'Mikrocitás vérkép normális RDW-vel, megtartott vasraktárral vagy magas vörösvértestszámmal — a vashiánynál inkább talasszémia-vonásra jellemző mintázat.',
    apnFocus: ['Családi és származási anamnézis', 'Korábbi, eredménytelen vaspótlás tisztázása', 'Felesleges vasterhelés kerülése'],
    further: ['Hemoglobin-elektroforézis', 'Ferritin, transzferrin-szaturáció', 'Vérkenet (target sejt)'],
    consult: 'Megerősítés és genetikai tanácsadás igénye esetén hematológiai konzultáció.',
  },
  {
    id: 'polycythaemia', name: 'Policitémia mintázata', sev: 'figyelendő',
    test: (d) => hi(d, 'hb') && (hi(d, 'htk') || hi(d, 'rbc')),
    summary: 'Emelkedett Hb, hematokrit és vörösvértestszám — lehet relatív (dehidráció) vagy valódi: krónikus hipoxia, illetve myeloproliferatív kórkép (policitémia vera).',
    apnFocus: ['Folyadékstátusz, testsúlyváltozás', 'Dohányzás, alvási apnoe, COPD feltárása', 'Thrombosis-jelek figyelése', 'Bőrpír, viszketés dokumentálása'],
    further: ['EPO-szint, JAK2-mutáció (hematológiai irány)', 'Oxigén-szaturáció, vérgáz', 'Thrombocyta, fehérvérsejtszám'],
    consult: 'Tartósan magas hematokrit, thrombosis vagy neurológiai panasz esetén.',
  },
]
