...
export interface CaseDoc {
  ...
  labs: { name?: string | null; value?: string | null; unit?: string | null; status?: string | null }[]
  scores: { score_name?: string | null; value?: number | null; band?: string | null }[]
  ekgs: { name?: string | null; category?: string | null; note?: string | null; assessment?: string | null }[]
...
function vitalLines(v?: Record<string, string> | null): string[] {
  if (!v) return []
  const out: string[] = []
  for (const def of ASSESS_VITALS) {
    const val = v[def.k]
    if (val) {
      const fl = vitalFlag(def.k, val)
      const tag = fl === 'red' ? ' [kritikus]' : fl === 'amber' ? ' [eltérés]' : ''
      out.push(`${def.label}: ${val} ${def.unit}${tag}`)
    }
  }
...
export function buildSummary(d: CaseDoc): string {
  ...
  L.push(`Fő panasz: ${d.complaint || MISSING}`)
  L.push(`Releváns anamnézis: ${d.background || MISSING}`)
  const dc = [d.diseaseName, d.contextName].filter(Boolean).join(' · ')
  L.push(`Betegség / kontextus: ${dc || MISSING}`, '')

  const vl = vitalLines(d.vitals)
  L.push('Vitális paraméterek:')
  L.push(vl.length ? vl.map((x) => `  • ${x}`).join('\n') : `  ${MISSING}`)
  L.push('')

  L.push('Releváns labor:')
  L.push(d.labs.length ? d.labs.map((l) => `  • ${l.name}: ${l.value ?? ''}${l.unit ? ' ' + l.unit : ''}${l.status ? ` (${l.status})` : ''}`).join('\n') : `  ${MISSING}`)
  L.push('')

  L.push('Score-ok:')
  L.push(d.scores.length ? d.scores.map((s) => `  • ${s.score_name}: ${s.value ?? '—'}${s.band ? ` · ${s.band}` : ''}`).join('\n') : `  ${MISSING}`)
  L.push('')

  L.push('EKG:')
  L.push(d.ekgs.length ? d.ekgs.map((e) => `  • ${e.name}${e.category ? ` (${e.category})` : ''}${e.note ? ` — ${e.note}` : ''}`).join('\n') : `  ${MISSING}`)
  L.push('')
...
export function buildSbar(d: CaseDoc): Sbar {
  const vl = vitalLines(d.vitals)
  const abnormalV = vl.filter((x) => /\[/.test(x))
  const abnormalL = d.labs.filter((l) => l.status && !/normál/i.test(l.status))

  const s = [
    `${d.title}.`,
    d.complaint ? `Fő panasz: ${d.complaint}.` : `Fő panasz: ${MISSING}.`,
    vl.length ? `Aktuális vitálisok: ${vl.join('; ')}.` : `Vitálisok: ${MISSING}.`,
  ].join(' ')

  const b = [
    d.background ? `Anamnézis: ${d.background}.` : `Anamnézis: ${MISSING}.`,
    d.diseaseName ? `Kapcsolódó betegség: ${d.diseaseName}.` : '',
  ].filter(Boolean).join(' ')
